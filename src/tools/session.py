...
10. 设计分阶段路线图（阶段有 KPI 与完成标准，不含时长字段；任务 schema：{name, description, priority}）
11. **起点对齐**：消费差距分析产出的 gap.start_level（当前可进入的企业层级），
    目标企业层级超起点时插入过渡阶段，不直接规划进不去的企业
12. **jd 三件套纪律**（知识光谱）：公司名与选人特点是公开常识，可直接写（如「某司欢迎双非」）；
...
13. **一次性展示全流程雏形**：把**所有阶段**（目标/KPI/里程碑/顺序理由）完整呈现给用户，
    不要只给第一阶段——用户需要全局地图才能判断路径是否合理
14. **打磨 loop**：与用户对话逐项打磨整份计划——调整阶段顺序、增删任务、
    修改 KPI/完成标准；打磨**未执行的未来计划**（用户还没想清楚、收益高），
    而非已执行部分（用户最清楚、收益低）。用户确认后才算定稿
15. 定稿 → `save_roadmap(roadmap_json=...)`（会自动产出路线图 HTML 交付物，双击即看）
