...
| `delete_profile` | 回收站式删除档案（需 `confirm="true"`，可恢复） | 用户确认废弃某档案时 |
| `list_trash` | 列出回收站中的档案（已删除、可恢复） | 想查看删除记录或恢复时 |
| `restore_profile` | 从回收站恢复档案（目标同名时拒绝覆盖） | 删除后想找回时 |
...
| 工具 | 作用 | 调用时机 |
|------|------|----------|
| `explore_goals` | 无目标时用三轴定位（能力×兴趣×真实市场数据）提出候选方向，对话引导选定 | 用户没有明确目标时 |
