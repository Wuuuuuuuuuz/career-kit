...
>  MCP 客户端注册时也需带上该环境变量。若嫌麻烦，直接跑真实目录亦可——档案管理功能（场景 B）本就是为此设计的。
...
**预期**：返回 ` 档案中已有目标：...` 提示（含 target_role），**不**直接返回方法论
...
**预期**：
- 返回 `路线图已保存` + 报告
- 含  依据待确认（intern 阶段占位未确认 → 提示确认「先占位后补JD」）
...
>  这里暴露一个真实交互：目前**没有"新建档案"工具**，alice 档案需通过先 `switch_profile` 到已存在的档案，或直接写文件。若你想造多份档案，最简单方式：当前 active=default 时，用 `merge_section` 直接给指定档案名写入（如测试用 `profile.merge_section("who", ..., "alice")`），或复制 default.json 为 alice.json。
